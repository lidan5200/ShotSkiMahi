...
namespace ShotSkiMahiD.Services
{
    public static class IniFileBL
    {
        public static Dictionary<string, string> ReadIniFile(string configFilePath)
        {
            ...
            {
                Debug.WriteLine($"[WARN] {DateTime.Now:HH:mm:ss} - INI配置文件读取异常: {ex.Message}");
                Debug.WriteLine($"       文件路径: {configFilePath}");
        ...
        public static void UpdataIni(string configFilePath, string systemCaching, string T0, string value)
        {
            ...
            {
                Debug.WriteLine($"[WARN] {DateTime.Now:HH:mm:ss} - INI配置文件写入异常: {ex.Message}");
                Debug.WriteLine($"       文件路径: {configFilePath}");
                Debug.WriteLine($"       写入位置: [{systemCaching}] {T0} = {value}");
                Debug.WriteLine($"       异常类型: {ex.GetType().Name}");
                if (ex.InnerException != null)
                    Debug.WriteLine($"       内部异常: {ex.InnerException.Message}");
            }
